...
home_dir = os.getenv("HOME") or "/home/krig"
...
function menuicon(name)
   local path = "/home/krig/.local/share/icons/" .. name
...
-- Disable mouse scroll on desktop to change tag
--    awful.button({ }, 4, awful.tag.viewnext),
--    awful.button({ }, 5, awful.tag.viewprev)
--))
-- }}}
...
function should_have_titlebar(c)
   return is_floating(c) and (c.type == "normal" or c.type == "dialog") and c.class ~= "Plugin-container"
end

function make_titlebar(c)
   if not should_have_titlebar(c) then
...
for s = 1, screen.count() do
    for t = 1, #tags[s] do
        tags[s][t]:connect_signal("property::layout", function ()
                                 ...
                                 for c in awful.client.iterate(f) do
                                    make_titlebar(c)
                                 end
        end)
    end
